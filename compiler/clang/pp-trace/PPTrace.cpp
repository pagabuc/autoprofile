...
// Option to specify the output file format
static cl::opt<bool> Json(
    "json", cl::init(false),
    cl::desc("Output in json rather than Yaml"));
...
static std::string quoteString(std::string s){
  return "\"" + s + "\"";
}


static int outputPPTraceJson(std::vector<CallbackCall> &CallbackCalls,
                         llvm::raw_ostream &OS) {
  for (std::vector<CallbackCall>::const_iterator I = CallbackCalls.begin(),
                                                 E = CallbackCalls.end();
       I != E; ++I) {
    ...
    const CallbackCall &Callback = *I;
    OS << "{" << quoteString("Callback") << ":" << quoteString(Callback.Name);
    ...
    for (auto AI = Callback.Arguments.begin(), AE = Callback.Arguments.end();
      ...
      OS << ", " << quoteString(Arg.Name) << ": ";

      if (Arg.Value.at(0) == '\"' or Arg.Name == "Range" or Arg.Name == "ConditionRange")
        OS << Arg.Value;
      else
        OS << quoteString(Arg.Value);
    }
...
int main(int Argc, const char **Argv) {
  ...
  if (!OutputFileName.size()) {
    if(Json)      
      HadErrors = outputPPTraceJson(CallbackCalls, llvm::outs());
    else
